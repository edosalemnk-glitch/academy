...
def _pdf_logo(canvas, doc):
    """En-tête PDF inspiré de la fiche INPP fournie, avec charte bleue institutionnelle."""
    width, height = A4
    canvas.saveState()
    blue = colors.HexColor("#1976b8")
    dark = colors.HexColor("#0d4f78")
    light = colors.HexColor("#eaf5fc")
    ...
    canvas.drawString(38*mm, height - 12*mm, "RÉPUBLIQUE DÉMOCRATIQUE DU CONGO")
    canvas.setFont("Helvetica-Bold", 9)
    canvas.drawString(38*mm, height - 17*mm, "INSTITUT NATIONAL DE PRÉPARATION PROFESSIONNELLE")
    canvas.setFont("Helvetica", 7.5)
    canvas.drawString(38*mm, height - 22*mm, "DIRECTION PROVINCIALE · INPP MATADI")
...
def procedure_pdf():
    """Fiche A4 portrait, structurée et colorée comme la fiche de renseignements de référence."""
    cfg = get_settings()
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=40*mm,
        bottomMargin=14*mm,
        title="Fiche de renseignements INPP Matadi",
        author="INPP Matadi",
    )
    ...
    blue = colors.HexColor("#1976b8")
    dark = colors.HexColor("#0d4f78")
    pale = colors.HexColor("#eef7fc")
    line = colors.HexColor("#9bc4df")
    grey = colors.HexColor("#5f6b73")

    cell = ParagraphStyle(
        "FicheCell", parent=styles["BodyText"], fontName="Helvetica",
        fontSize=6.4, leading=7.6, textColor=colors.HexColor("#26343d"),
        spaceAfter=0,
    )
    cell_center = ParagraphStyle(
        "FicheCellCenter", parent=cell, alignment=TA_CENTER,
    )
    service_cell = ParagraphStyle(
        "FicheService", parent=cell, fontName="Helvetica-Bold",
        fontSize=6.8, leading=8.2, alignment=TA_CENTER, textColor=dark,
    )
    head = ParagraphStyle(
        "FicheHead", parent=cell, fontName="Helvetica-Bold",
        fontSize=6.8, leading=8.2, alignment=TA_CENTER, textColor=colors.white,
    )
    title = ParagraphStyle(
        "FicheTitle", parent=styles["Title"], fontName="Helvetica-Bold",
        fontSize=18, leading=21, alignment=TA_CENTER, textColor=blue,
        spaceAfter=1.5*mm,
    )
    subtitle = ParagraphStyle(
        "FicheSubtitle", parent=cell, fontName="Helvetica-Bold",
        fontSize=7.5, leading=9, alignment=TA_CENTER, textColor=dark,
    )
    note = ParagraphStyle(
        "FicheNote", parent=cell, fontSize=7, leading=9, textColor=grey,
    )
    ...
    story = [
        ...
        Paragraph(
            "FORMATION PROFESSIONNELLE · FILIÈRES · MÉTIERS · DURÉES · FRAIS",
            subtitle,
        ),
        Spacer(1, 3*mm),
    ]
    ...
    story += [
        ...
        Paragraph(
            "Les frais matériels sont à payer une fois pendant la durée de la formation.",
            note,
        ),
        Spacer(1, 3*mm),
    ]

    # Tableau dense, proche de la fiche papier : service en bande colorée,
    # filière/métier/durée/frais sur une grille bleue lisible.
    data = [[
        Paragraph("SERVICE", head),
        Paragraph("FILIÈRE", head),
        Paragraph("MÉTIER", head),
        Paragraph("DURÉE", head),
        Paragraph("FRAIS MATÉRIELS", head),
    ]]
    row_groups = []
    row = 1
    for gi, group in enumerate(groups):
        service_name = group["name"].replace("Service ", "", 1)
        start_row = row
        service_color = [colors.HexColor("#dff1fb"), colors.HexColor("#eaf6fc"),
                         colors.HexColor("#d9eef9"), colors.HexColor("#f0f8fc")][gi % 4]
        for index, f in enumerate(group["rows"]):
            data.append([
                Paragraph(esc(service_name) if index == 0 else "", service_cell),
                Paragraph(esc(f["name"] or "—"), cell),
                Paragraph(esc(f["metier"] or "—"), cell),
                Paragraph(f"{f['duration_months']} mois", cell_center),
                Paragraph(f"{float(f['material_fee']):g} $", cell_center),
            ])
            row += 1
        row_groups.append((start_row, row - 1, service_color))

    table = LongTable(
        data,
        repeatRows=1,
        colWidths=[34*mm, 43*mm, 59*mm, 21*mm, 27*mm],
        splitByRow=1,
    )
    table_style = [
    ...
    for start_row, end_row, service_color in row_groups:
        table_style.append(("BACKGROUND",(0,start_row),(0,end_row),service_color))
        table_style.append(("BOX",(0,start_row),(0,end_row),0.5,blue))
        # Lignes de groupe légèrement accentuées.
        table_style.append(("LINEABOVE",(0,start_row),(-1,start_row),0.65,blue))
    for r in range(1, len(data)):
        if r % 2 == 0:
            table_style.append(("BACKGROUND",(1,r),(-1,r),colors.HexColor("#f8fbfd")))
    ...
    story += [
        ...
        Table([[
            Paragraph(
                "<b>NB :</b> Les frais payés ne sont pas remboursables.",
                ParagraphStyle("NB", parent=cell, fontName="Helvetica-Bold",
                               fontSize=7.2, leading=9, textColor=dark),
            ),
            Paragraph(
                "Les montants affichés proviennent du catalogue enregistré par le secrétariat.",
                note,
            ),
    ...
    response.headers["Content-Disposition"] = 'attachment; filename="fiche-renseignements-INPP-Matadi.pdf"'
    return response
